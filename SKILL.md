...
| **信源数** | **196 个，全覆盖**（Tier 0 Karpathy 精选 18 + Tier 1 企业核心 22 + Tier 2 大厂博客 22 + Tier 3 分析师 12 + Tier 4 国内中文 17 + Tier 4B 制造垂类 5 + Tier 4C 红杉系 3 + **Tier 4D 中国产业落地/甲方 15**（v5.3 新增）+ Tier 5 Karpathy 其余 74 + Tier 6 聚合 8 = 196） |
| **窗口策略** | **分层窗口**：Tier 0 默认 **7 天**（这层"每周 1-3 篇深度长文"特征），其他 Tier 严格 **24 小时** |
| **判断标尺** | 一个负责企业 AI 的 CXO/VP/PM/咨询顾问，会不会觉得"这条值得我知道"？ |

> **核心承诺：196 源全覆盖**。每个源在最终输出里必须有明确状态：
> - ✅ 抓到 N 篇 → 进入早报正文
...
## 2. 信源清单（v5 重构，196 个源 + Karpathy 全清单工具箱）
...
### 2.7b Tier 4D · 中国产业落地 & 甲方视角（常规抓 15 个，v5.3 新增）

> **为什么新增**：Tier 4（国内中文 17 源）全是 AI 科技/财经媒体（36氪/机器之心/量子位/智东西…），它们报道的天然是大模型厂商（**乙方**）的发布与融资。结果国内板块清一色"豆包/DeepSeek/智谱/Kimi 发了什么模型"，**几乎没有甲方落地视角**。Tier 4D 刻意补这个缺口——主攻"企业怎么用 AI / 制造业落地 / 行业实践 / 政策环境"。

| 子类 | 源 | 抓法 |
|---|---|---|
| **产业智库/甲方研究** | 甲子光年 / 亿欧 EqualOcean / 数智前线 / AI 掘金志(雷峰网产业) / 36氪 To B | 甲子光年·亿欧 fetch；数智前线公众号 search 兜底 |
| **制造业数字化** | e-works 数字化企业网 / 高工机器人 / 中国工业新闻网 / 智能制造 search 聚合 | e-works fetch；其余 search |
| **行业垂类落地** | 动脉网(医疗) / 盖世汽车(汽车) / 零售老板内参(零售) / 数字金融 search 聚合 | 公众号类 search 兜底 |
| **政策与官方** | 工信部 / 赛迪研究院 | fetch，频次低，0 命中正常 |

> Tier 4D 大量是微信公众号或反爬中文站，**默认 search 兜底**：`web_search "[源名] [关键词] 2026"`。命中目标是每天 2-5 篇**甲方/落地**视角内容，优先填充「🏢 企业部署案例」和「👥 组织变革」维度。
...
### Step 2：196 源全覆盖调度（v5.1 核心改造）

**v5.1 承诺**：196 个源全部点名，每个都有明确状态。但**抓的方式按源类型分发**——不是 196 次单 fetch（token 爆表），而是混合 fetch + 批量 search。
...
**第四层 = 跨天去重（v5.3 新增，解决"今天和昨天大量重叠"）**：

> 这是 v5.3 最重要的修复。前三层只在本次跑内部去重，无法阻止"同一事件连报 2-3 天"——咨询大报告（McKinsey/BCG/Gartner/PwC/a16z/Sequoia）、融资、IPO 传闻这类新闻，媒体会连续多天报道，每天 search 都命中、反复入选。

**执行方式**：
1. **写早报前必读历史**——读取过去 **2-3 天**已发布的早报正文（`reports/昨天.md`、`reports/前天.md`），用前三层同样的指纹算法，提取「已报道事件指纹清单」。
2. **逐条比对**当前候选与历史指纹：
   - **命中且无实质新进展** → ❌ 直接剔除，不进早报
   - **命中但有实质新进展**（新数据 / 新当事方 / 监管定性变化 / 价格变化 / 落地结果） → ✅ 保留，但**强制标注事件演进**（格式见下），且**默认不再占 Top 5**（除非进展本身是当日最重磅）
   - **未命中** → ✅ 正常进入候选池
3. **咨询报告类从严**：一份大报告（如 McKinsey《xxx》）原则上**只报一次**。后续除非出现「企业实际反应 / 第三方反驳 / 数据被推翻」等新角度，否则不再报，即使媒体仍在转载。

**事件演进标注格式**（命中历史但有新进展时必须加）：

```markdown
### [N] [📈 进展] 标题

- **来源** ｜ ...
- **📈 较昨日新增** ｜ 一句话说清"昨天报过什么 + 今天新增了什么"（如"昨日报白宫要求限量，今日新增 OpenAI 公开反对声明 + 三档模型命名"）
- ...
```

> 判断"实质新进展"的标尺：如果删掉这条，正在跟踪此事件的 CXO 会不会错过一个**新的决策输入**？会 → 保留并标注；不会（只是换家媒体复述同样的事实） → 剔除。
...
> 📡 **信源覆盖：196 / 196**（✅ 抓到 A 篇 / 🟡 已扫但 0 命中 B 个源 / ❌ 技术不可达 C 个源）
> ⭐ 平均质量分：X.X / 8.0
...
| **某批次 token 预算告急** | 截止当前批次，输出"已抓 X/196"+ 部分早报，附"剩余未抓的 Y 个源"说明 | **是** |
...
**本技能必须做的事**:每次手动触发,产出一份 30-50 篇企业 AI 资讯早报(按实际输出),翻译好、有"📌 这意味着"、可追溯、**196 源全覆盖审计**。
...
| **v5.3** | **针对"日报天天重叠"和"国内全是乙方动态"两个真实痛点的迭代:① 新增第 4 层『跨天去重』——写早报前强制读过去 2-3 天早报,已报事件无新进展则剔除、有进展则标注「📈 较昨日新增」,咨询大报告只报一次 ② 新增 Tier 4D『中国产业落地 & 甲方视角』15 源(产业智库/制造业数字化/行业垂类/政策官方),总源数 181→196 ③ 第 10 节新增第 16/17 条硬要求(跨天去重 + 中国甲方配额)** |
...
|---|---|---|
| **抓取覆盖率不可见** | 跑完了不知道自己跑了多少 | 强制每批次结束打印 "已抓 X/196" 自检 |
...
11. **196 源全覆盖审计是硬要求**——输出末尾必须含完整审计表,证明 196 个源每个都被点名。
...
15. **信源覆盖审计区可以保留**——但要从"开发证据"重新定位为"质量保证"。审计区在早报底部以**简洁版**呈现（最多 30 行），核心是让用户验证"哪些源你查了"，**不要长篇展示每个源的细节状态**——展示 4 类汇总即可（✅ 直接命中 N 个 / ✅ 间接命中 N 个 / 🟡 已扫无命中 N 个 / ❌ 技术不可达 N 个 = 总 196）。**展开的详细审计列表只在用户要求"给我看完整审计"时单独输出**。
16. **跨天去重是硬要求（v5.3）**——写早报**之前**必须读 `reports/昨天.md` 和 `reports/前天.md`，提取已报道事件指纹。当日候选与历史命中且**无实质新进展**的，一律剔除；有新进展的保留但必须加「📈 较昨日新增」标注且默认不占 Top 5。咨询大报告（McKinsey/BCG/Gartner/PwC/a16z/Sequoia 等）原则上只报一次。**严禁出现昨天报过、今天换家媒体复述同一事实的条目**——这是 v5.3 要消灭的头号问题。
17. **中国甲方视角配额（v5.3）**——Tier 4 国内媒体天然偏乙方（厂商发布/融资）。每天力争从 Tier 4D（甲子光年/亿欧/e-works/动脉网/盖世汽车/工信部等）挖到 **2-5 篇甲方/落地/制造/行业实践**内容，优先填「🏢 企业部署案例」「👥 组织变革」。国内板块不允许再清一色是"某厂发了什么模型"。
